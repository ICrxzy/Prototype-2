...
public class Game extends Canvas implements Runnable {
    ...
    public final SpriteSheet sprites, enemies;
    private boolean running = false;
    private final int width = 1200, height = 600;
    public int FPS;

    BufferedImage wall, floor, enemySprites, enemy, player, speed, fastFire, Healthpack;
    ...
    public Game(){
        ...
        enemySprites = loader.loadImage("/enemySprites.png");

        sprites = new SpriteSheet(SpriteSheet);
        enemies = new SpriteSheet(enemySprites);

        wall = sprites.grabImage(1, 1, 32, 32);
        floor = sprites.grabImage(2, 1, 32, 32);
        enemy = enemies.grabImage(2, 1, 32, 32);
    ...
    private void loadLevel(BufferedImage image) {
        ...
        for(int xx = 0; xx < w; xx++)
            for(int yy = 0; yy < h; yy++){
                ...
                if(red == 255 && blue == 0 && green == 0) handler.addObject(new Enemy(xx*32, yy*32, ID.Enemy, handler, hud, sprites));
